...
class ClassicLeagueLiveHandler(BaseHandler):
    ...
    async def get(self):
        ...
        try:
            ...
            elif parentPath == 'COMPSTATISTICS':
                leaugueId = self.pathInfo[2]
                response = self.getCompareStatisticsData(leaugueId)
    ...
    def getCompareStatisticsData(self, leagueId):

        cacheData = PickleUtil.getDataFromPickle(CoreUtils.getCompareStatsPickleFileName(CoreUtils.getGameweek(), leagueId))
        if cacheData:
            staleData = self.getStaleClassicLeagueStandings(leagueId)
            nameIdMap = {}
            idReplacedResponse = {}
            for member in staleData['members']:
                nameIdMap[int(member['info']['id'])] = member['info']['userName']
            for member in cacheData:
                transfers = []
                for transferGw in cacheData[member]['transfers']:
                    gwTransfers = cacheData[member]['transfers'][transferGw]
                    transfersDict = {}
                    transfersDict['gw'] = transferGw
                    transfersDict['transfers'] = []
                    for transfer in gwTransfers:
                        transfersDict['transfers'].append((self.getPlayerInfo(transfer[0])['name'], self.getPlayerInfo(transfer[1])['name']))
                    transfers.append(transfersDict)
                cacheData[member]['transfers'] = transfers
                idReplacedResponse[nameIdMap[member]] = cacheData[member]
            return idReplacedResponse
        return None
