...
class MembershipUpdateCriterion(Criterion):
    ...
    def isJobAlreadyComplete(self, gwNum):
        ...
        if log:
            completionDate = log.completionTime.date()
            todayDate = datetime.today().date()
            if completionDate == todayDate or gwNum == ActivityLog.gameweek:
                return True
        return False

    def isWithinMinsBeforeKickOffCacheUpdateTimeThreshold(self, gwNum):
        fixtureDetails = self.fplApi.getFixtures(gwNum=gwNum)
        startDatetime = MembershipUpdateCriterion.getGWStartDate(fixtureDetails)
...
if __name__ == '__main__':
    key = ''
    c = MembershipUpdateCriterion(key=key)
