    ...
    def run(self):
        ...
        for leagueId in leagueIds:
            ...
            for team in teamsInLeague:
                ...
                historicalTransferData = self.fplApi.getHistoricalTransfers(teamId)
                historicalData['transfers'] = historicalTransferData
                history[teamId] = historicalData
            stats = BasicAnalytics.runHistoricalAnalysisSuite(history, teamsInLeague)
            compareStats = BasicAnalytics.runCompareAnalysisSuite(history, teamsInLeague)
            PickleUtil.saveToPickle(stats, CoreUtils.getStatsPickleFileName(CoreUtils.getGameweek(), leagueId))
            PickleUtil.saveToPickle(compareStats, CoreUtils.getCompareStatsPickleFileName(CoreUtils.getGameweek(), leagueId))
